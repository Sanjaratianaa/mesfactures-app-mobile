...
export async function initDatabase(): Promise<SQLiteDBConnection> {
  ...
  // Exemple de création d'une table (on ajoutera les autres ensuite)
  await db.execute(`
...
export async function checkLocalUser(email: string, password: string) {
    ...
    const hash = res.values[0].mot_de_passe;
    const ok = await bcrypt.compare(password, hash);
    if (!ok) return false;
    // Retourne aussi la date_creation si besoin
    return { dateCreation: res.values[0].date_creation };
}